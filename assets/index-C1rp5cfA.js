...
type SlotKey = "flower" | "plume" | "sands" | "goblet" | "circlet"`,F=`interface IWeapon {
  ...
  = ${[...new Set(P)].sort().map(K=>`"${K}" //${o(`charNames_gen:${M(s.chars.LocationToCharacterKey(K),i)}`)}`).join(`
  ...
  = ${[...new Set(j)].sort().map(i=>`"${i}" //${o(`weaponNames_gen:${i}`)}`).join(`
