import { c as createSvgIcon, j as jsxRuntimeExports, z as useTranslation, b as jsx, a_ as languageCodeList, d as jsxs, ad as Trans, aX as MenuItem, aY as DropdownButton, r as reactExports, a$ as SillyContext, O as CardContent, b0 as Avatar, T as Typography, N as Divider, f as Box, b1 as default_1, b2 as default_1$1, ag as Button, a2 as DiscordIcon, h as CardThemed, b3 as SnowContext, ac as ReactGA, b4 as DatabaseCard } from "./index-B8aczfSH.js";
const AndroidIcon = createSvgIcon(/* @__PURE__ */ jsxRuntimeExports.jsx("path", {
  d: "m17.6 9.48 1.84-3.18c.16-.31.04-.69-.26-.85-.29-.15-.65-.06-.83.22l-1.88 3.24c-2.86-1.21-6.08-1.21-8.94 0L5.65 5.67c-.19-.29-.58-.38-.87-.2-.28.18-.37.54-.22.83L6.4 9.48C3.3 11.25 1.28 14.44 1 18h22c-.28-3.56-2.3-6.75-5.4-8.52zM7 15.25c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25zm10 0c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25z"
}), "Android");
const AppleIcon = createSvgIcon(/* @__PURE__ */ jsxRuntimeExports.jsx("path", {
  d: "M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"
}), "Apple");
const nativeLanguages = {
  chs: "简体中文",
  cht: "繁體中文",
  de: "Deutsch",
  en: "English",
  es: "Español",
  fr: "Français",
  id: "Bahasa Indonesia",
  it: "Italiano",
  ja: "日本語",
  ko: "한국어",
  pt: "Português",
  ru: "Русский язык",
  th: "ภาษาไทย",
  tr: "Türkçe",
  vi: "Tiếng Việt"
};
function LanguageCard() {
  const { t, i18n } = useTranslation(["ui", "settings"]);
  const onSetLanguage = (lang) => () => i18n.changeLanguage(lang);
  const currentLang = i18n.languages[0];
  return /* @__PURE__ */ jsx(
    DropdownButton,
    {
      fullWidth: true,
      title: t("settings:languageCard.languageFormat", {
        language: t(`languages:${currentLang}`)
      }),
      children: languageCodeList.map((lang) => /* @__PURE__ */ jsxs(
        MenuItem,
        {
          selected: currentLang === lang,
          disabled: currentLang === lang,
          onClick: onSetLanguage(lang),
          children: [
            /* @__PURE__ */ jsx(Trans, { i18nKey: `languages:${lang}` }),
            lang !== currentLang ? ` (${nativeLanguages[lang]})` : ""
          ]
        },
        lang
      ))
    }
  );
}
const silly_icon = "data:image/png;base64,";
function SillyCard() {
  const { silly, setSilly } = reactExports.useContext(SillyContext);
  const { t } = useTranslation(["ui", "settings"]);
  return /* @__PURE__ */ jsxs(CardThemed, { bgt: "light", children: [
    /* @__PURE__ */ jsxs(CardContent, { sx: { display: "flex", alignItems: "center", gap: 2 }, children: [
      /* @__PURE__ */ jsx(Avatar, { src: silly_icon }),
      /* @__PURE__ */ jsxs(Typography, { variant: "h5", children: [
        " ",
        t`sillyPageTitle`
      ] })
    ] }),
    /* @__PURE__ */ jsx(Divider, {}),
    /* @__PURE__ */ jsxs(CardContent, { children: [
      /* @__PURE__ */ jsx(Typography, { children: /* @__PURE__ */ jsxs(Trans, { t, i18nKey: "settings:sillyCard.desc", children: [
        "We ",
        /* @__PURE__ */ jsx("s", { children: "stole" }),
        " borrowed (with permission) all the character assets from ",
        /* @__PURE__ */ jsx("strong", { children: "Silly Wisher" }),
        " and gene splice it into Genshin Optimizer."
      ] }) }),
      /* @__PURE__ */ jsxs(Box, { display: "flex", gap: 2, pt: 2, children: [
        /* @__PURE__ */ jsx(
          Button,
          {
            onClick: () => setSilly(!silly),
            startIcon: silly ? /* @__PURE__ */ jsx(default_1, {}) : /* @__PURE__ */ jsx(default_1$1, {}),
            color: silly ? "success" : "secondary",
            children: silly ? t`settings:sillyCard.toggle.enabled` : t`settings:sillyCard.toggle.disabled`
          }
        ),
        /* @__PURE__ */ jsx(
          Button,
          {
            startIcon: /* @__PURE__ */ jsx(DiscordIcon, {}),
            color: "discord",
            href: "https://discord.com/invite/sillywisher",
            target: "_blank",
            children: "Discord"
          }
        ),
        /* @__PURE__ */ jsx(
          Button,
          {
            startIcon: /* @__PURE__ */ jsx(AppleIcon, {}),
            href: "https://apps.apple.com/lv/app/silly-wisher/id6444465724",
            target: "_blank",
            children: "App Store"
          }
        ),
        /* @__PURE__ */ jsx(
          Button,
          {
            startIcon: /* @__PURE__ */ jsx(AndroidIcon, {}),
            href: "https://play.google.com/store/apps/details?id=com.sketchi.sillywisher",
            target: "_blank",
            children: "Google Play"
          }
        )
      ] })
    ] })
  ] });
}
function SnowToggle() {
  const { snow, setSnow } = reactExports.useContext(SnowContext);
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxs(
    Button,
    {
      fullWidth: true,
      color: snow ? "success" : "secondary",
      onClick: () => setSnow(!snow),
      startIcon: snow ? /* @__PURE__ */ jsx(default_1, {}) : /* @__PURE__ */ jsx(default_1$1, {}),
      children: [
        "❄ ",
        t("settings:letsnow"),
        " ❄"
      ]
    }
  );
}
function PageSettings() {
  const { t } = useTranslation(["settings"]);
  ReactGA.send({ hitType: "pageview", page: "/setting" });
  return /* @__PURE__ */ jsxs(CardThemed, { sx: { my: 1 }, children: [
    /* @__PURE__ */ jsx(CardContent, { sx: { py: 1 }, children: /* @__PURE__ */ jsx(Typography, { variant: "subtitle1", children: /* @__PURE__ */ jsx(Trans, { t, i18nKey: "title" }) }) }),
    /* @__PURE__ */ jsx(Divider, {}),
    /* @__PURE__ */ jsxs(CardContent, { sx: { display: "flex", flexDirection: "column", gap: 2 }, children: [
      /* @__PURE__ */ jsx(LanguageCard, {}),
      /* @__PURE__ */ jsx(SnowToggle, {}),
      /* @__PURE__ */ jsx(SillyCard, {}),
      /* @__PURE__ */ jsx(DatabaseCard, {})
    ] })
  ] });
}
export {
  PageSettings as default
};
