...
type SlotKey = "flower" | "plume" | "sands" | "goblet" | "circlet"`,z=`interface IWeapon {
  ...
  | `)}`;return t(p,{children:[e(a,{gutterBottom:!0,variant:"h4",children:"StatKey"}),e(s,{text:r})]})}function X(){const{t:n}=g("artifactNames_gen"),i=`type ArtifactSetKey
  = ${[...new Set(R)].sort().map(r=>`"${r}" //${n(`artifactNames_gen:${r}`)}`).join(`
  | `)}`;return t(p,{children:[e(a,{gutterBottom:!0,variant:"h4",children:"ArtifactSetKey"}),e(s,{text:i})]})}function Y(){const{t:n}=g("charNames_gen"),i=N(),{gender:r}=P(),h=`type CharacterKey
  = ${[...new Set(V)].sort().map(b=>`"${b}" //${n(`charNames_gen:${j(i.chars.LocationToCharacterKey(b),r)}`)}`).join(`
  | `)}`;return t(p,{children:[e(a,{gutterBottom:!0,variant:"h4",children:"CharacterKey"}),e(s,{text:h})]})}function Z(){const{t:n}=g("weaponNames_gen"),i=`type WeaponKey
  = ${[...new Set(M)].sort().map(r=>`"${r}" //${n(`weaponNames_gen:${r}`)}`).join(`
  | `)}`;return t(p,{children:[e(a,{gutterBottom:!0,variant:"h4",children:"WeaponKey"}),e(s,{text:i})]})}function ee(){const{t:n}=g("material_gen"),i=`type MaterialKey
