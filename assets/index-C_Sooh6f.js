...
type SlotKey = "flower" | "plume" | "sands" | "goblet" | "circlet"`,F=`interface IWeapon {
  ...
  = ${[...new Set(P)].sort().map(i=>`"${i}" //${o(`weaponNames_gen:${i}`)}`).join(`
  ...
  = ${Object.keys(j.material).sort().map(i=>`"${i}" // ${o(`${i}.name`)}`).join(`
