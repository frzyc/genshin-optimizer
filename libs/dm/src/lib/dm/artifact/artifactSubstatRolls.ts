...
export const artifactSubstatRollCorrection: Record<
  string,
...
function getRolls(
  ...
  for (const current of allRolls) {
    ...
    if (index === -1)
      ...
      array[index].badChildCount === current.badChildCount &&
  ...
  for (const rolls of Object.values(outputRolls)) {
    for (const current of rolls) {
      possibleRolls.forEach((_, i) => {
        ...
        if (!accurateStrings.has(child.fastString)) {
          if (!(rarity in artifactSubstatRollCorrection))
            artifactSubstatRollCorrection[rarity] = {}
          if (!(key in artifactSubstatRollCorrection[rarity]))
            artifactSubstatRollCorrection[rarity][key] = {}
          artifactSubstatRollCorrection[rarity][key][child.fastString] =
            child.accurateString
        }
      })
    }
  }
...
export const artifactSubstatRollData = Object.fromEntries(
