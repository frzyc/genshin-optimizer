...
const weaponExcelConfigDataSrc = JSON.parse(readDMJSON("ExcelBinOutput/WeaponExcelConfigData.json")) as WeaponExcelConfigData[]
const weaponExcelConfigData = Object.fromEntries(weaponExcelConfigDataSrc.filter(({id}) => id in weaponIdMap).map(data =>
  [data.id, data])) as Record<WeaponId, WeaponExcelConfigData>
