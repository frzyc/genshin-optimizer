import { AnyNode, lookup, prod, RawTagMapEntries, subscript, sum, sumfrac } from '@genshin-optimizer/waverider'
import { final, one, hitEle, reader, todo } from '../util'
...
const crystallizeElemas = prod(40 / 9, sumfrac(final.q('eleMas'), 1400))
...
const transEleMasMulti = prod(16, sumfrac(final.q('eleMas'), 2000))
const trans = prod(transTypeMulti, transLvlMulti,
  sum(one, transEleMasMulti, reactionBonus),
  lookup(hitMode, {
    /* TODO: Check if can crit */
    hit: one, critHit: sum(one, final.q('critDMG_')),
    avgHit: sum(one, prod(final.q('critRate_'), final.q('critDMG_')))
  })
)
...
const ampMultiplier = prod(
  ...
  sum(one,
    prod(25 / 9, sumfrac(final.q('eleMas'), 1400)),
    reactionBonus)
)
