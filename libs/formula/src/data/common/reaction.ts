...
import { Data, percent, Reaction, reader, self, todo, transformativeReactions, TransReaction } from '../util'
...
const { final: { eleMas }, char: { lvl }, dmg } = self
const catReaction = todo, reactionBonus = todo

function eleMasMulti(a: number, b: number): NumNode {
  return sum(percent(1), prod(a, sumfrac(eleMas, b)), reactionBonus)
}
...
const transReactionMulti: Record<TransReaction, number> = {
  overloaded: 2, shattered: 1.5, electrocharged: 1.2, superconduct: 0.5,
  burning: 0.25, bloom: 2.0, burgeon: 3.0, hyperbloom: 3.0,
  swirl: 0.6,
}

const selfBuff = self
const ampBase = selfBuff.reaction.ampBase, transBase = self.reaction.transBase
const ampData: Data = [
  ampBase.addNode(eleMasMulti(25 / 9, 1400)),
  selfBuff.reaction.ampMulti.melt.cryo.addNode(prod(1.5, ampBase)),
  selfBuff.reaction.ampMulti.melt.pyro.addNode(prod(2.0, ampBase)),
  selfBuff.reaction.ampMulti.vaporize.pyro.addNode(prod(1.5, ampBase)),
  selfBuff.reaction.ampMulti.vaporize.hydro.addNode(prod(2.0, ampBase)),

  transBase.addNode(prod(subscript(lvl, transLvlMultis), eleMasMulti(16, 2000))),
  ...transformativeReactions.map(r =>
    selfBuff.dmg.final.name(r).addNode(prod(transReactionMulti[r], transBase, self.trans.critMulti))),
]

/**
  // Can Crit
  burning: { name: "Burning", multi: 0.25, variants: ["pyro", "dendro"], resist: "pyro", canCrit: true },
  bloom: { name: "Bloom", multi: 2, variants: ["dendro", "hydro"], resist: "dendro", canCrit: true },
  burgeon: { name: "Burgeon", multi: 3, variants: ["pyro"], resist: "dendro", canCrit: true },
  hyperbloom: { name: "Hyperbloom", multi: 3, variants: ["electro"], resist: "dendro", canCrit: true },
 */

export { ampData, crystallizeHit }
