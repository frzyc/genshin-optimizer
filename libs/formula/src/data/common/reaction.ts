import { cmpEq, lookup, NumNode, prod, subscript, sum, sumfrac } from '@genshin-optimizer/waverider'
import { Data, enemy, percent, self, selfBuff, TransReaction } from '../util'
...
const transReactionMulti: Record<Exclude<TransReaction, ''>, number> = {
  overloaded: 2, shattered: 1.5, electrocharged: 1.2, superconduct: 0.5,
  burning: 0.25, bloom: 2.0, burgeon: 3.0, hyperbloom: 3.0,
  swirl: 0.6,
}
const crittableTrans = new Set(['burning', 'bloom', 'burgeon', 'hyperbloom'])
...
const ampData: Data = [
  ...
  ...Object.entries(transReactionMulti).map(([reaction, multi]) =>
    selfBuff.dmg.final.name(reaction).add(prod(multi, transBase, ...(crittableTrans.has(reaction) ? [self.trans.critMulti.prod] : [])))),
