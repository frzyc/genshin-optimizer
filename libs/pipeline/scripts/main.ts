import { artifactIdMap, artifactSlotMap, CharacterData, CharacterId, characterIdMap, CharacterKey, Language, propTypeMap, QualityTypeMap, WeaponData, weaponIdMap, WeaponKey, weaponMap } from '../src/lib/pipeline'
...
import characterExpCurve from './DataminedModules/character/AvatarCurveExcelConfigData'
...
import weaponCurveExcelConfigData from './DataminedModules/weapon/WeaponCurveExcelConfigData'
...
const characterDataDump = Object.fromEntries(Object.entries(avatarExcelConfigData).filter(([charid, charData]) => charid in characterIdMap).map(([ci, charData]) => {
...
Object.entries(avatarExcelConfigData).forEach(([ci, charData]) => {
  if (!(ci in characterIdMap)) return
  const charid: CharacterId = ci as unknown as CharacterId
