...
export function extrapolateFloat(float: number, debug = false): number {
  if (Math.fround(float) !== float) {
    console.error(
      `Extrapolation error: ${float} is not a single-precision value`
    )
    return float
  }
  ...
  if (Math.floor(upper) === Math.floor(lower)) {
    ...
    for (;;) {
      ...
      if (uDigit !== lDigit) {
        if (upper === 0)
          console.warn(
            `Extrapolation error: extrapolated ${float} results in a midpoint}`
          )
        if (uDigit - lDigit !== 1) {
          if (debug)
            console.warn(
              `Extrapolation error: ambiguous value of ${float} (use ${digits.join(
                ''
              )}), the least significant digit could be ${lDigit + 1}`
            )
  ...
  if (Math.fround(parseFloat(string)) !== float)
    console.error(
      `Extrapolation error: extrapolated ${float} to an incorrect number ${string}`
    )
...
function roundingRange(float: number): [lower: number, upper: number] {
  ...
  const ulpOfOne = 1.1920928955078125e-7 // 2^-23, written as double
  const normalizedNext = normalized + ulpOfOne
  const normalizedPrev =
    normalized - (normalized === 1 ? ulpOfOne / 2 : ulpOfOne)
