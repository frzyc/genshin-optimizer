...
const DEST_PROJ_PATH =
  `${WORKSPACE_ROOT_PATH}/libs/sr-assets/src` as const
...
export default function loadImages() {
  ...
  dumpFile(`${WORKSPACE_ROOT_PATH}/libs/sr-assets-gen/src/lib/AssetData_gen.json`, AssetData)
  crawlObject(
    AssetData,
    [],
    (s) => typeof s === 'string',
    (icon, keys) => {
      copyFile(
        `${DM2D_PATH}/${icon}`,
        `${DEST_PROJ_PATH}/gen/${keys.slice(0, -1).join('/')}/${keys.at(-1)}.png`
      )
    }
  )
