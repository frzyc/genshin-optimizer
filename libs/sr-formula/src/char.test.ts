...
describe('character test', () => {
  it.each([
    ...
    expect(
      calc.compute(read({ src: charKey, qt: 'base', q: 'atk' }, undefined)).val
    ).toBeCloseTo(atk)
    expect(
      calc.compute(read({ src: charKey, qt: 'base', q: 'def' }, undefined)).val
    ).toBeCloseTo(def)
    expect(
      calc.compute(read({ src: charKey, qt: 'base', q: 'hp' }, undefined)).val
    ).toBeCloseTo(hp)
    expect(
      calc.compute(read({ src: charKey, qt: 'base', q: 'spd' }, undefined)).val
    ).toBeCloseTo(spd)
  })
})
