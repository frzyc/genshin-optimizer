 ...
 */
const fWeight: Record<SubstatKey, Weight> = {
  hp: 6,
  atk: 6,
  def: 6,
  hp_: 4,
    ...
    Partial<
      Record<
        ...
        Partial<
          Record<
            Weight,
            Partial<Record<Weight, Partial<Record<Weight, number>>>>
          >
        >
      >
    >
...
function probability(
  ...
  let numFillerSlots = 0
  {
    ...
    if (key in target) {
      ...
      if (maxLevelMainStat < target[key as keyof typeof target]!) return 0 // Main stat won't meet the target
...
function calculatePFillerRolls(
  ...
  const mainStatRatio = fWeight[mainStat as keyof typeof fWeight] ?? 0
  ...
  crawlObject(
    ...
    (prob: number, path: string[]) => {
      ...
      for (const key of path)
        currentCount[key as unknown as keyof typeof currentCount] += 1
