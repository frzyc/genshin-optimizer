...
export function ddx(
  ...
  switch (operation) {
    ...
    case 'div': {
      // TODO: Check if this is correct, it generated by Copilot and I'm not sure if it's correct
      // d/dx (a / b) = (b da - a db) / b^2
      const [a, b] = f.operands
      const da = ddx(a, binding, x)
      const db = ddx(b, binding, x)
      return frac(sum(prod(b, da), prod(-1, a, db)), prod(b, b))
    }
    case 'sub': {
      // TODO: Check if this is correct, it generated by Copilot and I'm not sure if it's correct
      // d/dx (a - b) = da - db
      const [a, b] = f.operands
      return sum(ddx(a, binding, x), prod(-1, ddx(b, binding, x)))
    }
    case 'avg': {
      // TODO: Check if this is correct, it generated by Copilot and I'm not sure if it's correct
      // d/dx avg(a, b) = (da + db) / 2
      const n = f.operands.length
      return frac(
        sum(...f.operands.map((fi) => ddx(fi, binding, x))),
        constant(n)
      )
    }
    default:
