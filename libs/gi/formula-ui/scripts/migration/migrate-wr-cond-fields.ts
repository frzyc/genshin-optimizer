...
function renderField(
  row: ReturnType<typeof parseWrFieldElement>
): string | null {
  if (!row) return null
  const indent = '          '
  if (row.kind === 'formula') {
...
function extractWrCondFields(key: string): Map<string, string[]> {
  ...
  while ((m = condTemRe.exec(wrSrc))) {
    ...
    for (const el of splitTopLevelElements(firstStateFields[1])) {
      const rendered = renderField(
        parseWrFieldElement(el, key, m[1] as TalentSection)
      )
...
function upsertCondFields(
  ...
  return src.replace(
    new RegExp(
      String.raw`(charConditionalDocument\(key, cond\.${condName}\)),`
    ),
    `$1, { ${fieldsBlock} }),`
  )
}
...
console.log(
  `Synced ${total} conditional field block(s) across ${keys.length} sheets`
)
