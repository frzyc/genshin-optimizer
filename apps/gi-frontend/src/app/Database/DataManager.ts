...
> {
  ...
  listeners: Dict<CacheKey, Callback<CacheKey, CacheValue>[]> = {}
  anyListeners: Callback<CacheKey, CacheValue>[] = []
  ...
  followAny(callback: Callback<CacheKey, CacheValue>): () => void {
    this.anyListeners.push(callback)
    return () => {
      this.anyListeners = this.anyListeners.filter((cb) => cb !== callback)
    }
  }
  follow(key: CacheKey, callback: Callback<CacheKey, CacheValue>) {
...
type Callback<Arg, Obj> = (key: Arg, reason: TriggerString, object: Obj) => void
