...
export function artifactFilterConfigs(effFilterSet: Set<SubstatKey> = new Set(allSubstatKeys)): FilterConfigs<keyof FilterOption, ICachedArtifact> {
  return {
    ...
    locations: (art, filter, filters) => filters.showEquipped || filter.includes(art.location),
    showEquipped: (art, filter, filters) => !!art.location === filter || !filter && filters.locations.includes(art.location),
    showInventory: (art, filter) => !art.location === filter,
