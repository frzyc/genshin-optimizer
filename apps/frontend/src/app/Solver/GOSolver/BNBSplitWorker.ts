...
type Filter = {
  ...
  maxConts: Record<SlotKey, number>[]
...
function weightedSum(
  ...entries: readonly (readonly [number, DynStat])[]
): DynStat
function weightedSum(
  ...entries: readonly (readonly [number, DynStat])[]
): DynStat {
...
export function linearUpperBound(
  nodes: OptNode[],
  arts: ArtifactsBySlot
): Linear[] {
  const cents = weightedSum(
    [1, arts.base],
    ...Object.values(arts.values).map(
      (arts) =>
        [
          1 / arts.length,
          weightedSum(...arts.map((art) => [1, art.values] as const)),
        ] as const
    )
  )
  ...
  const nodeRanges = computeNodeRange(
    [...minMaxes.keys()],
    computeFullArtRange(arts)
  )
  ...
  return customMapFormula<Context, Linear, OptNode>(
    nodes,
    upper,
    (f, context, _map) => {
      const { operation } = f
      const map: (op: OptNode, c?: Context) => Linear = (op, c = context) =>
        _map(op, c)
      ...
      switch (operation) {
        case 'const':
          return { $c: f.value }
        case 'read':
          return { $c: 0, [f.path[1]]: 1 }
        case 'add':
          return weightedSum(...f.operands.map((op) => [1, map(op)] as const))
        case 'min':
        case 'max': {
        ...
        case 'res': {
          ...
          if (min < 0 && max < 1.75)
            return weightedSum([1, { $c: 1 }], [-0.5, x])
        ...
        case 'threshold': {
          ...
          const slope =
            (pass - fail) / (isFirstHalf ? thresh - min : max - thresh)
        ...
        case 'mul': {
          ...
          const factor = cents.reduce(
            (accu, cent, i) =>
              accu + (cent >= 0 ? ranges[i].max : ranges[i].min) / cent,
            0
          )
          const prod = cents.reduce(
            (a, b) => (a * factor * b) / lins.length,
            coeff / factor
          )
          return weightedSum(
            ...lins.map((op, i) => [prod / cents[i], op] as const)
          )
        }
