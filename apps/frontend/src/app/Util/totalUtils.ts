...
export function catTotal<T extends CatTotalKey>(keys: readonly T[], cb: (ct: Record<T, Entry>) => void) {
  const ct = catTotalObj(keys)
  cb(ct)
  return catTotalToStringObj(ct)
}

function catTotalObj<T extends CatTotalKey>(keys: readonly T[]): Record<T, Entry> {
