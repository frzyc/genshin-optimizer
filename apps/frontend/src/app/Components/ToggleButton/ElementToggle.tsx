import { allElementKeys, ElementKey } from "@genshin-optimizer/consts";
...
export default function ElementToggle({ value, totals, onChange, ...props }: ElementToggleProps) {
  ...
  return <ToggleButtonGroup exclusive value={value} {...props}>
    {allElementKeys.map(ele => <SolidColoredToggleButton key={ele} value={ele} sx={{ p: sm ? 1 : undefined, minWidth: sm ? 0 : "6em", display: "flex" }} selectedColor={ele} onClick={() => onChange(elementHandler(value, ele))}>
