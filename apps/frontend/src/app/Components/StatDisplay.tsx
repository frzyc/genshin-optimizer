...
export function StatWithUnit({ statKey, disableIcon = false }: { statKey: StatKey, disableIcon?: boolean }) {
  return <Box component="span" display="flex" alignItems="center" gap={1}>{!disableIcon && <StatIcon statKey={statKey} iconProps={iconInlineProps}/>}<span>{KeyMap.get(statKey)}{KeyMap.unit(statKey)}</span></Box>
}
