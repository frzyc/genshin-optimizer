...
export const ambiguousLevel = (level) =>
  level !== maxLevel && ascensionMaxLevel.includes(level)
export const ambiguousLevelLow = (level) =>
  level !== maxLevelLow && ascensionMaxLevelLow.includes(level)
...
export function validateLevelAsc(
  level: number,
  ascension: Ascension
): { level: number; ascension: Ascension } {
