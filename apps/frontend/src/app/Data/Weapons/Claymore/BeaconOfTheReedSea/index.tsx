...
const afterSkill_atk_ = equal(input.weapon.key, key, equal(condAfterSkill, "on",
    subscript(input.weapon.refineIndex, afterSkillAtkArr, { ...KeyMap.info("atk_") })
  )
)

const afterDmgAtkArr = [0.2, 0.25, 0.3, 0.35, 0.4]
const [condAfterDmgPath, condAfterDmg] = cond(key, "afterDmg")
const afterDmg_atk_ = equal(input.weapon.key, key, equal(condAfterDmg, "on",
    subscript(input.weapon.refineIndex, afterDmgAtkArr, { ...KeyMap.info("atk_") })
  )
)

const noShieldHpArr = [0.32, 0.40, 0.48, 0.56, 0.64]
const [condNoShieldPath, condNoShield] = cond(key, "noShield")
const noShield_hp_ = equal(input.weapon.key, key, equal(condNoShield, "on",
    subscript(input.weapon.refineIndex, noShieldHpArr, { ...KeyMap.info("hp_") })
  )
)
