...
export default function CustomTooltip({ xLabel, xUnit, yLabel, yUnit, selectedPoint, setSelectedPoint, addBuildToList, ...tooltipProps }: CustomTooltipProps) {
  ...
  const currentlyEquipped = artifactsBySlot && allArtifactSlotKeys.every(slotKey => artifactsBySlot[slotKey]?.id === data.get(input.art[slotKey].id).value)

  const generLabel = useMemo(() => selectedPoint?.generBuildNumber !== undefined && `#${selectedPoint?.generBuildNumber}`, [selectedPoint])
  const graphLabel = useMemo(() => selectedPoint?.graphBuildNumber !== undefined && <Trans t={t} i18nKey="graphBuildLabel" count={selectedPoint?.graphBuildNumber}>Graph #{{ count: selectedPoint?.graphBuildNumber + 1 }}</Trans>,
  [selectedPoint, t])
      ...
      <CardContent>
        <Stack gap={1}>
          ...
          <BootstrapTooltip title={selectedPoint.highlighted ? t("tcGraph.buildAlreadyInList") : ""} placement="top">
            <span>
              <Button sx={{ width: "100%" }} disabled={selectedPoint?.graphBuildNumber !== undefined} color="info" onClick={() => addBuildToList(selectedPoint.artifactIds)}>{t("addBuildToList")}</Button>
            </span>
          </BootstrapTooltip>
        </Stack>
      </CardContent>
