...
const embeds = JSON.parse(import.meta.env.NX_URLS_GUIDES ?? "[]") as string[]

export default function VidGuideCard() {
  const { t } = useTranslation(["page_home", "ui"])
  if (!embeds.length) return null
  return <CardDark>
    <CardHeader title={<Typography variant="h5" component={Link} color="inherit" href={import.meta.env.NX_URL_YOUTUBE_TUTPL} target="_blank" rel="noopener">{t`vidGuideCard.title`}</Typography>} avatar={<YouTube fontSize="large" />} />
    <Divider />
    <CardContent>
