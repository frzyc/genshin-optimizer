...
export default function InventoryCard() {
  ...
  return <CardDark>
    ...
    <CardContent sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
      <CardLight>
        <CardActionArea
          sx={{ display: "flex", justifyContent: "space-between", p: 2, gap: 1, flexWrap: "wrap" }}
          component={RouterLink}
          to="/characters"
        >
          <Chip
            label={<strong>{t<string>(`ui:tabs.characters`)} {characterTotal}</strong>}
            icon={<People />}
            sx={{ flexBasis: smaller ? "100%" : "auto", flexGrow: 1, cursor: "pointer" }}
          ...
          {Object.entries(characterTally).map(([ele, num]) =>
            <Chip
              ...
              icon={<FontAwesomeSvgIcon icon={elementSvg[ele]} />}
      ...
      <CardLight>
        ...
        >
          <Chip
            label={<strong>{t<string>(`ui:tabs.weapons`)} {weaponTotal}</strong>}
      ...
      <CardLight>
        ...
        >
          <Chip
            label={<strong>{t<string>(`ui:tabs.artifacts`)} {artifactTotal}</strong>}
          ...
          {Object.entries(artifactTally).map(([slotKey, num]) =>
            <Chip
              ...
              sx={{ flexGrow: 1, cursor: "pointer" }}
              color={num ? "success" : "secondary"}
              icon={<FontAwesomeSvgIcon icon={slotIconSVG[slotKey]} />}
              label={<strong>{num}</strong>}
            />
          )}
        </CardActionArea>
