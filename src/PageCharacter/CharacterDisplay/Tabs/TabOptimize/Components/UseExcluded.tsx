...
export default function UseExcluded({ buildSettingsDispatch, useExcludedArts, disabled, artsDirty }) {
  ...
  return <Button fullWidth onClick={() => buildSettingsDispatch({ useExcludedArts: !useExcludedArts })} disabled={!numExcludedArt || disabled} startIcon={useExcludedArts ? <CheckBox /> : <CheckBoxOutlineBlank />} color={useExcludedArts ? "success" : "primary"}>
    <Box>
