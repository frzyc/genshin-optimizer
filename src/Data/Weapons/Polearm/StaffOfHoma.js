...
const weapon = {
  ...
  passiveDescription: (refineIndex, charFinalStats) => {
    ...
    return <span>HP increased by {refinementVals_hp[refineIndex]}%. Additionally, provides an ATK Bonus based on {refinementVals_hp_atk[refineIndex]}% of the wielder's Max HP{DisplayPercent(refinementVals_hp_atk[refineIndex], charFinalStats, "hp_final")}. When the wielder's HP is less than 50%, this ATK bonus is increased by an additional {refinementVals_hp_atk_add[refineIndex]}% of Max HP{DisplayPercent(refinementVals_hp_atk_add[refineIndex], charFinalStats, "hp_final")}.</span>
  },
  ...
  conditional: {
    ...
    maxStack: 1,
    stats: (refineIndex) => ({
      modifiers: { atk_final: { hp_final: refinementVals_hp_atk_add[refineIndex] / 100 } }
    })
  },
}
