...
const weapon = {
  ...
  passiveDescription: (refineIndex) => `Increases Shield Strength by ${refinementVals[refineIndex]}%. Scoring hits on opponents increases ATK by ${refinementVals[refineIndex]}% for 8s. Max 5 stacks. Can only occur once every 0.3s. While protected by a shield, this ATK increase effect is increased by 100%.`,
  ...
  stats: (refineIndex) => ({
    pow_shield: refinementVals[refineIndex]
  }),
  conditional: [{
    type: "weapon",
    condition: "Hits without shield",
    sourceKey: "VortexVanquisher",
    maxStack: 5,
    stats: (refineIndex) => ({
