...
export function dataObjForWeaponSheet(
  ...
  const mainStatNode = infoMut(sum(prod(mainStat.base, subscript(input.weapon.lvl, weaponCurves[mainStat.curve])), subscript(input.weapon.asc, gen.ascension.map(x => x.addStats[mainStat.type] ?? 0))), KeyMap.keyToInfo(mainStat.type))
  ...
  if (subStat) {
    ...
    const substatNode = infoMut(prod(subStat.base, subscript(input.weapon.lvl, weaponCurves[subStat.curve])), KeyMap.keyToInfo(subStat.type))
    merging.push({
