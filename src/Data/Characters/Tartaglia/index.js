...
const char = {
  ...
  talent: {
    ...
    skill: {
      ...
      document: [{
        ...
        ...data.skill.hitArr.map((percentArr, i) => ({
          ...
          formulaText: stats =>
            <span>{percentArr[stats.tlvl.skill]}% {Stat.printStat(getTalentStatKey("normal", stats, true), stats)}</span>,
    ...
    burst: {
      ...
      document: [{
        ...
        fields: [{
          ...
          formulaText: stats =>
            <span>{data.burst.melee[stats.tlvl.burst]}% {Stat.printStat(getTalentStatKey("burst", stats), stats)}</span>,
          formula: formula.burst.melee,
          variant: stats => getTalentStatKeyVariant("burst", stats),
        }, {
          text: "Skill DMG: Ranged",
          formulaText: stats =>
            <span>{data.burst.ranged[stats.tlvl.burst]}% {Stat.printStat(getTalentStatKey("burst", stats), stats)}</span>,
    ...
    constellation1: {
      ...
      img: c1,
      document: [{ text: <span>Decrease the CD of Foul Legacy: Raging Tide by 20%</span> }],
    },
    constellation2: {
      name: "Foul Legacy: Understream",
      img: c2,
      document: [{ text: <span>When opponents affected by Riptide are defeated. Tartaglia regenerates 4 Elemental Energy.</span> }],
    },
    constellation3: {
