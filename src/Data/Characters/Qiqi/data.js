...
const data = {
  ...
  normal: {
    ...
    twoHit: [false, false, true, true, false],
  },
  charged: {
...
const formula = {
  ...
  skill: {
    ...
    hitregen: (tlvl) => {
      ...
      const atk = data.skill.hitregen.atk[tlvl];
      return [s => (atk * s.characterATK + base) * s.heal_multi, ["characterATK", "heal_multi"]]
    },
    continuousregen: (tlvl) => {
      const base = data.skill.continuousregen.base[tlvl];
      const atk = data.skill.continuousregen.atk[tlvl];
      return [s => (atk * s.characterATK + base) * s.heal_multi, ["characterATK", "heal_multi"]]
    },
  },
  burst: {
    ...
    healing: (tlvl) => {
      ...
      const atk = data.burst.healing.atk[tlvl];
      return [s => (atk * s.characterATK + base) * s.heal_multi, ["characterATK", "heal_multi"]]
    },
  },
}
