...
export function characterFilterConfigs(database: ArtCharDatabase, characterSheets: Record<CharacterKey, CharacterSheet>): CharacterFilterConfigs {
  return {
    element: (ck, filter) => !filter || filter.includes(characterSheets?.[ck]?.elementKey) || (ck === "Traveler" && !database._getChar(ck as CharacterKey)) || (ck === "Traveler" && filter.includes(database._getChar(ck as CharacterKey)?.elementKey)),
