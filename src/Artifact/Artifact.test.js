...
test('getAllArtifactSetEffectsObj', () => {
  const stats1 = {}
  expect(Artifact.getAllArtifactSetEffectsObj(stats1).RetracingBolide).toEqual({ 2: { powShield_: 35 } })
  const stats2 = { conditionalValues: { artifact: { RetracingBolide: { set4: [1] } } } }
  expect(Artifact.getAllArtifactSetEffectsObj(stats2).RetracingBolide).toEqual({ 2: { powShield_: 35 }, 4: { normal_dmg_: 40, charged_dmg_: 40 } })
})
